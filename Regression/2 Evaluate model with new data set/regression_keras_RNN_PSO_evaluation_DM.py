...
class NeuralNetworkOptimizer:
    def __init__(self, X_train, y_train, X_test, y_test, meas_sec, epochs=1000, show_plot="no"):
        """
    ...
    def create_baseline(self):
        """
        Create a baseline recurrent neural network (LSTM) model.

        Returns:
            keras.models.Sequential: Compiled Keras model for the recurrent neural network.
        ...
        model.add(LSTM(units                 = 15, 
                       ...
                       recurrent_initializer = initializers.HeNormal(seed=42),
                       bias_initializer      = initializers.RandomNormal(seed=42),
        ...
        model.add(LSTM(units                 = 9, 
                       ...
                       kernel_initializer    = initializers.TruncatedNormal(seed=42),
                       recurrent_initializer = initializers.TruncatedNormal(seed=42),
                       bias_initializer      = initializers.GlorotUniform(seed=42),
    ...
    def evaluate_nn(self):
        ...
        This method trains the recurrent neural network, calculates evaluation scores, and displays plots.
        ...
        self.machine_learning_best_scores(estimator, "Recurrent Neural Network")
        self.plot_learning_curve(self.create_baseline(), "Recurrent Neural Network")
        self.plot_best_results(estimator, "Recurrent Neural Network")
        self.plot_regression_result(estimator, "Recurrent Neural Network")
    ...
    def machine_learning_best_scores(self, estimator, model_name):
        ...
        Args:
            estimator (keras.models.Sequential): Trained recurrent neural network model.
        ...
        train1      = r2_score(self.y_train[:,0],preds_train[:,0])
        test1       = r2_score(self.y_test[:,0],preds_test[:,0])
        
        train2      = mean_squared_error(self.y_train[:,0],preds_train[:,0],squared=False)
        test2       = mean_squared_error(self.y_test[:,0],preds_test[:,0],squared=False)
    
        train3      = mean_absolute_error(self.y_train[:,0],preds_train[:,0])
        test3       = mean_absolute_error(self.y_test[:,0],preds_test[:,0])
        
        train4      = explained_variance_score(self.y_train[:,0],preds_train[:,0])
        test4       = explained_variance_score(self.y_test[:,0],preds_test[:,0])
    ...
    def plot_best_results(self, mod_best_param, model_name):
        ...
        best_y_pred_test = mod_best_param.predict(self.X_test)[:,0]
        best_y_test      = self.y_test[:,0]
        ...
        if self.show_plot == "yes":
            ...
            ax.plot(x_axis, best_y_pred_test, "s", color="green", label="Prediction values", markersize=4, alpha=0.5)
            ax.set_ylim(80, 98)
